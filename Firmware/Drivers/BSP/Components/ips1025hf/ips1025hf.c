/**
  ...
  * @brief  IPS1025HF (High Efficiency High Side Switch with extended
  *         diagnostic and smart driving for capacitive loads) driver
  ******************************************************************************
  * @attention
  *
...
/** @defgroup IPS1025HF_Global_Variables IPS1025HF Global Variables
  * @{
  */

/** Common driver structure which lists IPS1025HF functions */
IPS1025HF_CommonDrv_t IPS1025HF_COMMON_Driver =
{
  IPS1025HF_Init,
  IPS1025HF_DeInit,
  IPS1025HF_ReadID,
  IPS1025HF_GetCapabilities,
  IPS1025HF_GetFaultStatus,
  IPS1025HF_GetChannelStatus,
  IPS1025HF_SetChannelStatus,
  IPS1025HF_GetAllChannelStatus,
  IPS1025HF_SetAllChannelStatus,
  IPS1025HF_GetChannelFreq,
  IPS1025HF_SetChannelFreq,
  IPS1025HF_GetChannelDc,
  IPS1025HF_SetChannelDc,
  IPS1025HF_GetPwmEnable,
  IPS1025HF_SetPwmEnable,
  IPS1025HF_PwmTick,
  IPS1025HF_GetCtrlPinStatus,
  IPS1025HF_SetCtrlPinStatus,
  IPS1025HF_SetNchDrvSyncEnable
};
...
/** @defgroup IPS1025HF_Private_Variables IPS1025HF Private Variables
  * @{
  */
...
/** @defgroup IPS1025HF_Private_Function_Prototypes IPS1025HF Private Function Prototypes
  * @{
  */

static int32_t IPS1025HF_Initialize(IPS1025HF_Object_t *pObj);
int32_t IPS1025HF_NchDrvPwmSync(IPS1025HF_Object_t *pObj, int8_t ChanId);
...
/** @defgroup IPS1025HF_Functions_Definition IPS1025HF Functions Definition
  * @{
  */
...
/**
  * @brief  Initialize the IPS1025HF switch
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t IPS1025HF_Init(IPS1025HF_Object_t *pObj)
{
  ...
  {
    if (IPS1025HF_Initialize(pObj) != IPS_SWITCH_OK)
    {
    ...
    {
      IPS_NbInstances++;
      if (IPS_NbInstances > IPS1025HF_INSTANCES_NBR)
      {
        status = IPS_SWITCH_ERROR;
      }
      else
...
/**
  * @brief  Deinitialize the IPS1025HF switch
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t IPS1025HF_DeInit(IPS1025HF_Object_t *pObj)
{
  ...
  if (pObj->is_initialized == 1U)
  {
    ...
    {
      (void)IPS1025HF_SetPwmEnable(pObj, (uint8_t)loop, 0U);
    }

    /* Disable all channels */
    (void)IPS1025HF_SetAllChannelStatus(pObj, 0U);
...
/**
  * @brief  Get IPS1025HF switch capabilities
  * @param  pObj pointer to the device object
...
int32_t IPS1025HF_GetCapabilities(IPS1025HF_Object_t *pObj, IPS1025HF_Capabilities_t *pCapabilities)
{
  ...
  if (pObj->chipId == IPS1025HF_CHIP_ID)
  {
    pCapabilities->nbChannels = IPS1025HF_MAX_NB_CHANNELS;
    switch(pObj->Instance)
    {
      case 0U:
        pCapabilities->firstCtrlPin = IPS1025HF_0_FIRST_CONTROL;
        break;

      case 1U:
        pCapabilities->firstCtrlPin = IPS1025HF_1_FIRST_CONTROL;
        break;
    ...
    pCapabilities->nbCtrlPins = IPS1025HF_MAX_NB_CONTROLS;
  }
  else
  {
    status = IPS_SWITCH_ERROR;
...
int32_t IPS1025HF_NchDrvPwmSync(IPS1025HF_Object_t *pObj, int8_t ChanId)
{
  ...
  if (pObj->chanPwmTimPeriodLow[ChanId] >= minPeriodLow)
  {
    ...
    {
      status = pObj->IO.WritePin(&pObj->Pin, IPS1025HF_0_NCH_DRV, 0U);
    ...
    else
    {
      ...
      {
        status = pObj->IO.WritePin(&pObj->Pin, IPS1025HF_0_NCH_DRV, 1U);
      }
    ...
    for (chanId = (int8_t)(pObj->nbChannels) - 1; chanId >= 0; chanId--)
    {
      ...
      if ((pObj->isPwmEnabled & (0x1U << (uint8_t)chanId)) != 0U)
      {
        ...
        if (pObj->NchDrv_isPwmSyncEnabled[chanId] != 0U)
        {
          ...
          {
            status_NchDrv = IPS1025HF_NchDrvPwmSync(pObj, chanId);
          }
        }
      }
      else
      {
        ...
        if (pObj->NchDrv_isPwmSyncEnabled[chanId] != 0U)
        {
          ...
          {
            status_NchDrv = pObj->IO.WritePin(&pObj->Pin, IPS1025HF_0_NCH_DRV, 0U);
          }
        }
      }
...
int32_t IPS1025HF_SetChannelFreq(IPS1025HF_Object_t *pObj, uint8_t ChanId, uint16_t ChanFreq)
{
  ...
  {
    uint16_t newFreq;
    if (ChanFreq > IPS1025HF_MAX_CHAN_FREQ)
    {
       newFreq = IPS1025HF_MAX_CHAN_FREQ;
    }
    else
    {
      newFreq = ChanFreq;
...
static int32_t IPS1025HF_Initialize(IPS1025HF_Object_t *pObj)
{
  ...
  if (IPS1025HF_SetAllChannelStatus(pObj, 0U) != IPS_SWITCH_OK)
  {
    ret = IPS_SWITCH_ERROR;
  }
  return ret;
