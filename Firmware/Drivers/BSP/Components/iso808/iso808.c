/**
  ...
  * @brief   ISO808 (Galvanic isolated octal high-side smart power solid
  *          state-relay) driver
  ******************************************************************************
  * @attention
  *
...
/** @defgroup ISO808_Global_Variables ISO808 Global Variables
  * @{
  */

/** Common driver structure which lists ISO808 functions */
ISO808_CommonDrv_t ISO808_COMMON_Driver =
{
  ISO808_Init,
  ISO808_DeInit,
  ISO808_ReadID,
  ISO808_GetCapabilities,
  ISO808_GetFaultStatus,
  ISO808_GetChannelStatus,
  ISO808_SetChannelStatus,
  ISO808_GetAllChannelStatus,
  ISO808_SetAllChannelStatus,
  ISO808_GetChannelFreq,
  ISO808_SetChannelFreq,
  ISO808_GetChannelDc,
  ISO808_SetChannelDc,
  ISO808_GetPwmEnable,
  ISO808_SetPwmEnable,
  ISO808_PwmTick,
  ISO808_GetCtrlPinStatus,
  ISO808_SetCtrlPinStatus,
  ISO808_SetOperatingMode
};
...
/** @defgroup ISO808_Private_Function_Prototypes ISO808 Private Function Prototypes
  * @{
  */

static int32_t ISO808_Initialize(ISO808_Object_t *pObj);
...
/** @defgroup ISO808_Functions_Definition ISO808 Functions Definition
  * @{
  */
...
/**
  * @brief  Initialize the ISO808 switch
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t ISO808_Init(ISO808_Object_t *pObj)
{
  ...
  {
    if (ISO808_Initialize(pObj) != IPS_SWITCH_OK)
    {
    ...
    {
      IPS_NbInstances++;
      if (IPS_NbInstances > ISO808_INSTANCES_NBR)
      {
        status = IPS_SWITCH_ERROR;
      }
      else
...
/**
  * @brief  Deinitialize the ISO808 switch
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t ISO808_DeInit(ISO808_Object_t *pObj)
{
  ...
  if (pObj->is_initialized == 1U)
  {
    ...
    {
      (void)ISO808_SetPwmEnable(pObj, (uint8_t)loop, 0U);
    }

     /* Disable all channels */
    (void)ISO808_SetAllChannelStatus(pObj, 0U);
...
/**
  * @brief  Get ISO808 switch capabilities
  * @param  pObj pointer to the device object
...
int32_t ISO808_GetCapabilities(ISO808_Object_t *pObj, ISO808_Capabilities_t *pCapabilities)
{
  ...
  if (pObj->chipId == ISO808_CHIP_ID)
  {
    pCapabilities->nbChannels = ISO808_MAX_NB_CHANNELS;
    pCapabilities->nbCtrlPins = ISO808_MAX_NB_CONTROLS;
    pCapabilities->firstCtrlPin = ISO808_0_FIRST_CONTROL;
  }
  else
  {
    status = IPS_SWITCH_ERROR;
...
int32_t ISO808_PwmTick(ISO808_Object_t *pObj)
{
  ...
  if (pObj->isPwmEnabled != 0U)
  {
    ...
    if (pObj->ctrlMode == (uint8_t)ISO808_SCM)
    {
      flagToBeChanged = 0U;
    }
    ...
    if (pObj->ctrlMode == (uint8_t)ISO808_SCM)
    {
      if (chanEnBitmapChangeFlag != 0U)
      {
        ...
        ret = pObj->IO.WritePin(&pObj->Pin, ISO808_0_CTRL_SYNCH, 1U);
        if (ret != IPS_SWITCH_OK)
        {
          status = IPS_SWITCH_ERROR;
        }
        ret = pObj->IO.WritePin(&pObj->Pin, ISO808_0_CTRL_LOAD, 1U);
        if (ret != IPS_SWITCH_OK)
        {
          status = IPS_SWITCH_ERROR;
        }
        for (loop = 0U; loop < ISO808_TW_LOAD_WIDTH; loop++)
        {
          /* Loop needed to match tW(LOAD) */
          ret = pObj->IO.WritePin(&pObj->Pin, ISO808_0_CTRL_LOAD, 0U);
          if (ret != IPS_SWITCH_OK)
          {
            status = IPS_SWITCH_ERROR;
          }
    ...
    if (pObj->ctrlMode == (uint8_t)ISO808_SCM)
    {
      if (chanEnBitmapChangeFlag != 0U)
      {
        ...
        for (loop = 0U; loop < ISO808_TSU_LOAD_WIDTH; loop++)
        {
        ...
        for (loop = 0U; loop < ISO808_TW_SYNC_WIDTH; loop++)
        {
          /* Loop needed to keep SYNC low for more than 20 us, that is tw(SYNC) */
          ret = pObj->IO.WritePin(&pObj->Pin, ISO808_0_CTRL_SYNCH, 0U);
          if (ret != IPS_SWITCH_OK)
          {
            status = IPS_SWITCH_ERROR;
          }
...
int32_t ISO808_SetAllChannelStatus(ISO808_Object_t *pObj, uint8_t ChanStatusBitmap)
{
  ...
  else
  {
    ...
    if (pObj->ctrlMode == (uint8_t)ISO808_SCM)
    {
      ...
      for (waitLoop = 0U; waitLoop < ISO808_TW_LOAD_WIDTH; waitLoop++)
      {
        /* Loop needed to match tW(LOAD) */
        ret = pObj->IO.WritePin(&pObj->Pin, ISO808_0_CTRL_LOAD, 0U);
        if (ret != IPS_SWITCH_OK)
    ...
    if (pObj->ctrlMode == (uint8_t)ISO808_SCM)
    {
      ...
      /* SYNCH low (Preparing to transfer to output stage) */
      /* SYNCH high (Effective transfer to output stage) */
      for (waitLoop = 0U; waitLoop < ISO808_TSU_LOAD_WIDTH; waitLoop++)
      {
        /* Loop needed to match tSU(LOAD) */
        ret = pObj->IO.WritePin(&pObj->Pin, ISO808_0_CTRL_LOAD, 1U);
        if (ret != IPS_SWITCH_OK)
      ...
      for (waitLoop = 0U; waitLoop < ISO808_TW_SYNC_WIDTH; waitLoop++)
      {
        /* Loop needed to keep SYNC low for more than 20 us, that is tw(SYNC) */
        ret = pObj->IO.WritePin(&pObj->Pin, ISO808_0_CTRL_SYNCH, 0U);
        if (ret != IPS_SWITCH_OK)
...
int32_t ISO808_SetChannelFreq(ISO808_Object_t *pObj, uint8_t ChanId, uint16_t ChanFreq)
{
  ...
  {
    uint16_t newFreq;
    if (ChanFreq > ISO808_MAX_CHAN_FREQ)
    {
       newFreq = ISO808_MAX_CHAN_FREQ;
    }
    else
    {
      newFreq = ChanFreq;
...
int32_t ISO808_SetOperatingMode(ISO808_Object_t *pObj, uint8_t opMode)
{
  ...
  if (opMode == (uint8_t)ISO808_DCM)
  {
    ...
    pObj->ControlPinsBitmap &= ~(0x1U << (ISO808_0_CTRL_SYNCH - pObj->firstCtrl));
    ret = pObj->IO.WritePin(&pObj->Pin, ISO808_0_CTRL_LOAD, 0U);
    if (ret != IPS_SWITCH_OK)
    {
      status = IPS_SWITCH_ERROR;
    }
    pObj->ControlPinsBitmap &= ~(0x1U << (ISO808_0_CTRL_LOAD - pObj->firstCtrl));

    pObj->ctrlMode = (uint8_t)ISO808_DCM;
  }
  else
  {
    ...
    pObj->ctrlMode = (uint8_t)ISO808_SCM;
  }
...
static int32_t ISO808_Initialize(ISO808_Object_t *pObj)
{
  ...
  if (ISO808_SetAllChannelStatus(pObj, 0U) != IPS_SWITCH_OK)
  {
    ret = IPS_SWITCH_ERROR;
  }
  return ret;
