...
/** @defgroup ISO808A_Global_Variables ISO808A Global Variables
  * @{
  */

extern void *RELAY_CompObj[ISO808A_INSTANCES_NBR];

/** Common driver structure which lists ISO808A functions */
ISO808A_CommonDrv_t ISO808A_COMMON_Driver =
{
  ISO808A_Init,
  ISO808A_DeInit,
  ISO808A_ReadID,
  ISO808A_GetCapabilities,
  ISO808A_GetFaultStatus,
  ISO808A_GetFaultRegister,
  ISO808A_GetChannelStatus,
  ISO808A_SetChannelStatus,
  ISO808A_GetCtrlPinStatus,
  ISO808A_SetCtrlPinStatus,
  ISO808A_GetAllChannelStatus,
  ISO808A_SetAllChannelStatus,
  ISO808A_GetChannelFreq,
  ISO808A_SetChannelFreq,
  ISO808A_GetChannelDc,
  ISO808A_SetChannelDc,
  ISO808A_GetPwmEnable,
  ISO808A_SetPwmEnable,
  ISO808A_PwmTick,
  ISO808A_GuardTimerTick,
  ISO808A_QueueChannelStatus,
  ISO808A_QueueAllChannelStatus,
  ISO808A_SendQueuedChannelStatus,
  ISO808A_GetFaultRegister_DaisyChain,
  ISO808A_PwmTick_DaisyChain,
  ISO808A_GuardTimerTick_DaisyChain
};
...
/** @defgroup ISO808A_Private_Variables ISO808A Private Variables
  * @{
  */
...
/** @defgroup ISO808A_Private_Function_Prototypes ISO808A Private Function Prototypes
  * @{
  */

static int32_t ISO808A_Initialize(ISO808A_Object_t *pObj);
int32_t ISO808A_WriteBytes(ISO808A_Object_t *pObj, uint8_t *pByteToTransmit, uint8_t *pReceivedByte, uint8_t nbDevices);
...
/** @defgroup ISO808A_Functions_Definition ISO808A Functions Definition
  * @{
  */
...
/**
  * @brief  Initialize the ISO808A relay
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t ISO808A_Init(ISO808A_Object_t *pObj)
{
  ...
  {
    if (ISO808A_Initialize(pObj) != IPS_RELAY_OK)
    {
    ...
    {
      IPS_NbInstances++;
      if (IPS_NbInstances > ISO808A_INSTANCES_NBR)
      {
        status = IPS_RELAY_ERROR;
      }
      else
...
/**
  * @brief  Deinitialize the ISO808A relay
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t ISO808A_DeInit(ISO808A_Object_t *pObj)
{
  if (pObj->is_initialized == 1U)
  {
    ...
    (void)ISO808A_SetAllChannelStatus(pObj, 0U);
...
/**
  * @brief  Get ISO808A relay capabilities
  * @param  pObj pointer to the device object
...
int32_t ISO808A_GetCapabilities(ISO808A_Object_t *pObj, ISO808A_Capabilities_t *pCapabilities)
{
  ...
  if (pObj->chipId == ISO808A_CHIP_ID)
  {
    pCapabilities->nbChannels = ISO808A_MAX_NB_CHANNELS;
    switch(pObj->Instance)
    {
      case 0U:
        pCapabilities->firstCtrlPin = ISO808A_0_FIRST_CONTROL;
        break;

      case 1U:
        pCapabilities->firstCtrlPin = ISO808A_1_FIRST_CONTROL;
        break;
    ...
    pCapabilities->nbCtrlPins = ISO808A_MAX_NB_CONTROLS;
  }
  else
  {
    status = IPS_RELAY_ERROR;
...
int32_t ISO808A_GetFaultRegister(ISO808A_Object_t *pObj, uint8_t *pFaultRegister)
{
  ...
  *pFaultRegister = 0xFFU;
  if (pObj->isPwmEnabled == 0U)
  {
    ...
    status = ISO808A_WriteBytes(pObj, &aSpiTxBuffer[0], &aSpiRxBuffer[0], 1U);
...
int32_t ISO808A_GetFaultRegister_DaisyChain(uint16_t *pFaultRegister)
{
  ...
  *pFaultRegister = 0xFFFFU;
  ...
  if (status == IPS_RELAY_OK)
  {
    ...
    for (Instance=0; Instance < ISO808A_INSTANCES_NBR; Instance++)
    {
      pObj = (ISO808A_Object_t*)RELAY_CompObj[Instance];
  ...
  if (status == IPS_RELAY_OK)
  {
    ...
    {
      pObj = (ISO808A_Object_t*)RELAY_CompObj[Instance];
      aSpiTxBuffer[ISO808A_INSTANCES_NBR - 1U - Instance] = pObj->ChanSteadyStateBitmap;
    }

    pObj = (ISO808A_Object_t*)RELAY_CompObj[0];
    SpiTxRxBytes = 2U;
    do
    {
      spiPreemptionByIsr = FALSE;
    ...
    status = ISO808A_WriteBytes(pObj, &aSpiTxBuffer[0], &aSpiRxBuffer[0], SpiTxRxBytes);

    /* Get 2-bytes Fault Register data from SPI Rx Buffer */
    FaultRegVal = 0U;
    for (Instance=0; Instance < ISO808A_INSTANCES_NBR; Instance++)
    {
      FaultRegVal |= ((uint16_t)(aSpiRxBuffer[ISO808A_INSTANCES_NBR - 1U - Instance]) << (8U*Instance));
    }
...
int32_t ISO808A_SetChannelFreq(ISO808A_Object_t *pObj, uint8_t ChanId, uint16_t ChanFreq)
{
  ...
  {
    uint16_t newFreq;
    if (ChanFreq > ISO808A_MAX_CHAN_FREQ)
    {
       newFreq = ISO808A_MAX_CHAN_FREQ;
    }
    else
    {
      newFreq = ChanFreq;
...
/**
  ...
  * @retval 0 in case of success, an error code otherwise
  */
int32_t ISO808A_SetPwmEnable(ISO808A_Object_t *pObj, uint8_t ChanId, uint8_t EnablePwm)
{
  int32_t status;
...
int32_t ISO808A_PwmTick(ISO808A_Object_t *pObj)
{
  ...
  if (pObj->isPwmEnabled != 0U)
  {
    ...
    /*  Update channels first to minimize latency */
    status = ISO808A_SetAllChannelPwmStatus(pObj,  pObj->nextChanEnBitmap | pObj->ChanSteadyStateBitmap);
...
int32_t ISO808A_PwmTick_DaisyChain(void)
{
  ...
  if (RELAY_System_PWMEnable_bitmap != 0U)
  {
    ...
    /*  Update channels first to minimize latency */
    (void)ISO808A_SendQueuedPwmChannelStatus();
    ...
    for (Instance=0; Instance < ISO808A_INSTANCES_NBR; Instance++)
    {
      ...
      if (pObj->isPwmEnabled != 0U)
      {
        ...
        {
          status = ISO808A_QueueAllChannelPwmStatus(pObj, pObj->nextChanEnBitmap);
        }
        else
        {
          status = ISO808A_QueueAllChannelPwmStatus(pObj, pObj->nextChanEnBitmap | pObj->ChanSteadyStatePwmBitmap);
        }
      }
      else
      {
        status = ISO808A_QueueAllChannelPwmStatus(pObj, pObj->ChanSteadyStatePwmBitmap);
      }
    }
...
int32_t ISO808A_GuardTimerTick(ISO808A_Object_t *pObj)
{
  ...
  if (spiLockTime > 0)
  {
    spiLockTime -= (int16_t)(ISO808A_GUARD_TIMER_TICK_IN_US*5U);
  }
  else
  {
...
int32_t ISO808A_SetAllChannelPwmStatus(ISO808A_Object_t *pObj, uint8_t ChanStatusBitmap)
{
  ...
  else
  {
    ...
    if (RELAY_System_OpMode == (uint8_t)ISO808A_DCM)
    {
      SpiTxRxBytes = 2U;
    }
    else
...
int32_t ISO808A_SetChannelStatus(ISO808A_Object_t *pObj, uint8_t ChanId, uint8_t ChanStatus)
{
  ...
  if (ChanId < pObj->nbChannels)
  {
    ...
    if (pObj->isPwmEnabled != 0U)
    {
      status = ISO808A_SetAllChannelPwmStatus(pObj,  pObj->ChanSteadyStateBitmap);
    }
    else
    {
      status = ISO808A_SetAllChannelStatus(pObj,  pObj->ChanSteadyStateBitmap);
    }
  }
  else
  {
...
/**
  ...
  *             sending done by calling ISO808A_SendQueuedChannelStatus.
  ...
  *             specified before calling ISO808A_SendQueuedChannelStatus.
  * @param      pObj pointer to the device object
  * @param      ChanId Id of the targeted channel
  * @param      ChanStatus the channel status to be set
  * @retval     0 in case of success, an error code otherwise
...
int32_t ISO808A_QueueChannelStatus(ISO808A_Object_t *pObj, uint8_t ChanId, uint8_t ChanStatus)
{
  ...
  if (ChanId < pObj->nbChannels)
  {
    ...
    aSpiTxBuffer[ISO808A_INSTANCES_NBR - 1U - deviceId] = pObj->ChanSteadyStateBitmap;
...
int32_t ISO808A_QueueAllChannelStatus(ISO808A_Object_t *pObj, uint8_t newChanBitmap)
{
  ...
  else
  {
    ...
    aSpiTxBuffer[ISO808A_INSTANCES_NBR - 1U - deviceId] = newChanBitmap;
...
int32_t ISO808A_SendQueuedChannelStatus(void)
{
  ...
  if (queuedBitmap != FALSE)
  {
    ...
    /* using the shift byte feature available in ISO808A (SDO-0 goes to SDI-1): first byte will be passed to */
...
/**
  ...
  *             synchronous sending done by calling ISO808A_SendQueuedPwmChannelStatus.
  *             Any call to functions that use the SPI between the calls of
  *             - ISO808A_QueueAllChannelPwmStatus
  *             - ISO808A_SendQueuedChannelStatus
  *             will corrupt the queue.
  *             A command for each device of the daisy chain must be
  *             specified before calling ISO808A_SendQueuedChannelStatus.
...
int32_t ISO808A_QueueAllChannelPwmStatus(ISO808A_Object_t *pObj, uint8_t newChanBitmap)
{
  ...
  {
    aSpiTxBufferPwm[ISO808A_INSTANCES_NBR - 1U - deviceId] = newChanBitmap;
...
int32_t ISO808A_SendQueuedPwmChannelStatus(void)
{
  ...
  if (queuedPwmBitmap != FALSE)
  {
    ...
    status = ISO808A_WriteBytes(pObj, &aSpiTxBufferPwm[0], &aSpiRxBufferPwm[0], SpiTxRxBytes);
...
static int32_t ISO808A_Initialize(ISO808A_Object_t *pObj)
{
  ...
  {
    case 0U:
      ctrl_id = ISO808A_0_OUT_EN;
      break;

    case 1U:
      ctrl_id = ISO808A_1_OUT_EN;
      break;
  ...
  {
    status = ISO808A_SetCtrlPinStatus(pObj, ctrl_id, 0U);
  }
  return status;
}
...
int32_t ISO808A_WriteBytes(ISO808A_Object_t *pObj, uint8_t *pByteToTransmit, uint8_t *pReceivedByte, uint8_t nbDevices)
{
  ...
  {
    spiLockTime = (int16_t)pObj->timingTcss - (int16_t)(ISO808A_GUARD_TIMER_TICK_IN_US*5U);
