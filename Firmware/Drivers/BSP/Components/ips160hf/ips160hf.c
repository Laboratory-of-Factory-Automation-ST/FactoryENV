/**
  ...
  * @brief  IPS160HF (Single Channel High Side Switch 2.5A) driver
  ******************************************************************************
  * @attention
  *
...
/** @defgroup IPS160HF_Global_Variables IPS160HF Global Variables
  * @{
  */

/** Common driver structure which lists IPS160HF functions */
IPS160HF_CommonDrv_t IPS160HF_COMMON_Driver =
{
  IPS160HF_Init,
  IPS160HF_DeInit,
  IPS160HF_ReadID,
  IPS160HF_GetCapabilities,
  IPS160HF_GetFaultStatus,
  IPS160HF_GetChannelStatus,
  IPS160HF_SetChannelStatus,
  IPS160HF_GetAllChannelStatus,
  IPS160HF_SetAllChannelStatus,
  IPS160HF_GetChannelFreq,
  IPS160HF_SetChannelFreq,
  IPS160HF_GetChannelDc,
  IPS160HF_SetChannelDc,
  IPS160HF_GetPwmEnable,
  IPS160HF_SetPwmEnable,
  IPS160HF_PwmTick,
  IPS160HF_GetCtrlPinStatus,
  IPS160HF_SetCtrlPinStatus,
  IPS160HF_SetNchDrvSyncEnable
};
...
/** @defgroup IPS160HF_Private_Variables IPS160HF Private Variables
  * @{
  */
...
/** @defgroup IPS160HF_Private_Function_Prototypes IPS160HF Private Function Prototypes
  * @{
  */

static int32_t IPS160HF_Initialize(IPS160HF_Object_t *pObj);
int32_t IPS160HF_NchDrvPwmSync(IPS160HF_Object_t *pObj, int8_t ChanId);
...
/** @defgroup IPS160HF_Functions_Definition IPS160HF Functions Definition
  * @{
  */
...
/**
  * @brief  Initialize the IPS160HF switch
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t IPS160HF_Init(IPS160HF_Object_t *pObj)
{
  ...
  {
    if (IPS160HF_Initialize(pObj) != IPS_SWITCH_OK)
    {
    ...
    {
      IPS_NbInstances++;
      if (IPS_NbInstances > IPS160HF_INSTANCES_NBR)
      {
        status = IPS_SWITCH_ERROR;
      }
      else
...
/**
  * @brief  Deinitialize the IPS160HF switch
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t IPS160HF_DeInit(IPS160HF_Object_t *pObj)
{
  ...
  if (pObj->is_initialized == 1U)
  {
    ...
    {
      (void)IPS160HF_SetPwmEnable(pObj, (uint8_t)loop, 0U);
    }

    /* Disable all channels */
    (void)IPS160HF_SetAllChannelStatus(pObj, 0U);
...
/**
  * @brief  Get IPS160HF or IPS61HF switch capabilities
  * @param  pObj pointer to the device object
...
int32_t IPS160HF_GetCapabilities(IPS160HF_Object_t *pObj, IPS160HF_Capabilities_t *pCapabilities)
{
  ...
  if (pObj->chipId == IPS160HF_CHIP_ID)
  {
    pCapabilities->nbChannels = IPS160HF_MAX_NB_CHANNELS;
    switch(pObj->Instance)
    {
      case 0U:
        pCapabilities->firstCtrlPin = IPS160HF_0_FIRST_CONTROL;
        break;

      case 1U:
        pCapabilities->firstCtrlPin = IPS160HF_1_FIRST_CONTROL;
        break;

      case 2U:
        pCapabilities->firstCtrlPin = IPS160HF_2_FIRST_CONTROL;
        break;

      case 3U:
        pCapabilities->firstCtrlPin = IPS160HF_3_FIRST_CONTROL;
        break;
    ...
    pCapabilities->nbCtrlPins = IPS160HF_MAX_NB_CONTROLS;
  }
  else
  {
    status = IPS_SWITCH_ERROR;
...
int32_t IPS160HF_NchDrvPwmSync(IPS160HF_Object_t *pObj, int8_t ChanId)
{
  ...
  if (pObj->chanPwmTimPeriodLow[ChanId] >= minPeriodLow)
  {
    ...
    {
      status = pObj->IO.WritePin(&pObj->Pin, IPS160HF_0_NCH_DRV, 0U); // TO DO: @GC
    ...
    else
    {
      ...
      {
        status = pObj->IO.WritePin(&pObj->Pin, IPS160HF_0_NCH_DRV, 1U);
      }
      else if(pObj->NchDrv_TimTickCnt[ChanId] == pObj->NchDrv_TimTickOff[ChanId])
      {
        status = pObj->IO.WritePin(&pObj->Pin, IPS160HF_0_NCH_DRV, 0U);
      }
      else
      {
    ...
    for (chanId = (int8_t)(pObj->nbChannels) - 1; chanId >= 0; chanId--)
    {
      ...
      if ((pObj->isPwmEnabled & (0x1U << (uint8_t)chanId)) != 0U)
      {
        ...
        if (pObj->NchDrv_isPwmSyncEnabled[chanId] != 0U)
        {
          ...
          {
            status_NchDrv = IPS160HF_NchDrvPwmSync(pObj, chanId);
          }
        }
      }
      else
      {
        ...
        if (pObj->NchDrv_isPwmSyncEnabled[chanId] != 0U)
        {
          ...
          {
            status_NchDrv = pObj->IO.WritePin(&pObj->Pin, IPS160HF_0_NCH_DRV, 0U);
          }
        }
      }
...
int32_t IPS160HF_SetChannelFreq(IPS160HF_Object_t *pObj, uint8_t ChanId, uint16_t ChanFreq)
{
  ...
  {
    uint16_t newFreq;
    if (ChanFreq > IPS160HF_MAX_CHAN_FREQ)
    {
       newFreq = IPS160HF_MAX_CHAN_FREQ;
    }
    else
    {
      newFreq = ChanFreq;
...
/**
  * @brief  Initialize the IPS160HF or IPS161HF switch
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
static int32_t IPS160HF_Initialize(IPS160HF_Object_t *pObj)
{
  ...
  if (IPS160HF_SetAllChannelStatus(pObj, 0U) != IPS_SWITCH_OK)
  {
    ret = IPS_SWITCH_ERROR;
  }
  return ret;
