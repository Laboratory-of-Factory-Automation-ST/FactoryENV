/**
  ...
  * @brief   IPS2050H (High Efficiency Dual High Side) driver
  ******************************************************************************
  * @attention
  *
...
/** @defgroup IPS2050H_Global_Variables IPS2050H Global Variables
  * @{
  */

/** Common driver structure which lists IPS2050H functions */
IPS2050H_CommonDrv_t IPS2050H_COMMON_Driver =
{
  IPS2050H_Init,
  IPS2050H_DeInit,
  IPS2050H_ReadID,
  IPS2050H_GetCapabilities,
  IPS2050H_GetFaultStatus,
  IPS2050H_GetChannelStatus,
  IPS2050H_SetChannelStatus,
  IPS2050H_GetAllChannelStatus,
  IPS2050H_SetAllChannelStatus,
  IPS2050H_GetChannelFreq,
  IPS2050H_SetChannelFreq,
  IPS2050H_GetChannelDc,
  IPS2050H_SetChannelDc,
  IPS2050H_GetPwmEnable,
  IPS2050H_SetPwmEnable,
  IPS2050H_PwmTick
};
...
/** @defgroup IPS2050H_Private_Variables IPS2050H Private Variables
  * @{
  */
...
/** @defgroup IPS2050H_Private_Function_Prototypes IPS2050H Private Function Prototypes
  * @{
  */

static int32_t IPS2050H_Initialize(IPS2050H_Object_t *pObj);
...
/** @defgroup IPS2050H_Functions_Definition IPS2050H Functions Definition
  * @{
  */
...
/**
  * @brief  Initialize the IPS2050H switch
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t IPS2050H_Init(IPS2050H_Object_t *pObj)
{
  ...
  {
    if (IPS2050H_Initialize(pObj) != IPS_SWITCH_OK)
    {
    ...
    {
      IPS_NbInstances++;
      if (IPS_NbInstances > IPS2050H_INSTANCES_NBR)
      {
        status = IPS_SWITCH_ERROR;
      }
      else
...
/**
  * @brief  Deinitialize the IPS2050H switch
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t IPS2050H_DeInit(IPS2050H_Object_t *pObj)
{
  ...
  if (pObj->is_initialized == 1U)
  {
    ...
    {
        (void)IPS2050H_SetPwmEnable(pObj, (uint8_t)loop, 0U);
    }

     /* Disable all channels */
    (void)IPS2050H_SetAllChannelStatus(pObj, 0U);
...
/**
  * @brief  Get IPS2050H switch capabilities
  * @param  pObj pointer to the device object
...
int32_t IPS2050H_GetCapabilities(IPS2050H_Object_t *pObj, IPS2050H_Capabilities_t *pCapabilities)
{
  ...
  if (pObj->chipId == IPS2050H_CHIP_ID)
  {
...
int32_t IPS2050H_SetChannelFreq(IPS2050H_Object_t *pObj, uint8_t ChanId, uint16_t ChanFreq)
{
  ...
  {
    uint16_t newFreq;
    if (ChanFreq > IPS2050H_MAX_CHAN_FREQ)
    {
       newFreq = IPS2050H_MAX_CHAN_FREQ;
    }
    else
    {
      newFreq = ChanFreq;
...
static int32_t IPS2050H_Initialize(IPS2050H_Object_t *pObj)
{
  ...
  if (IPS2050H_SetAllChannelStatus(pObj, 0U) != IPS_SWITCH_OK)
  {
    ret = IPS_SWITCH_ERROR;
  }
  return ret;
