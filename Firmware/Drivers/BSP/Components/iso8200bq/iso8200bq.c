/**
  ...
  * @brief   ISO8200BQ (Galvanic isolated octal high-side smart power solid
  *          state-relay) driver
  ******************************************************************************
  * @attention
  *
...
/** @defgroup ISO8200BQ_Global_Variables ISO8200BQ Global Variables
  * @{
  */

/** Common driver structure which lists ISO8200BQ functions */
ISO8200BQ_CommonDrv_t ISO8200BQ_COMMON_Driver =
{
  ISO8200BQ_Init,
  ISO8200BQ_DeInit,
  ISO8200BQ_ReadID,
  ISO8200BQ_GetCapabilities,
  ISO8200BQ_GetFaultStatus,
  ISO8200BQ_GetChannelStatus,
  ISO8200BQ_SetChannelStatus,
  ISO8200BQ_GetAllChannelStatus,
  ISO8200BQ_SetAllChannelStatus,
  ISO8200BQ_GetChannelFreq,
  ISO8200BQ_SetChannelFreq,
  ISO8200BQ_GetChannelDc,
  ISO8200BQ_SetChannelDc,
  ISO8200BQ_GetPwmEnable,
  ISO8200BQ_SetPwmEnable,
  ISO8200BQ_PwmTick,
  ISO8200BQ_GetCtrlPinStatus,
  ISO8200BQ_SetCtrlPinStatus,
  ISO8200BQ_SetOperatingMode
};
...
/** @defgroup ISO8200BQ_Private_Variables ISO8200BQ Private Variables
  * @{
  */
...
/** @defgroup ISO8200BQ_Private_Function_Prototypes ISO8200BQ Private Function Prototypes
  * @{
  */

static int32_t ISO8200BQ_Initialize(ISO8200BQ_Object_t *pObj);
...
/** @defgroup ISO8200BQ_Functions_Definition ISO8200BQ Functions Definition
  * @{
  */
...
/**
  * @brief  Initialize the ISO8200BQ switch
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t ISO8200BQ_Init(ISO8200BQ_Object_t *pObj)
{
  ...
  {
    if (ISO8200BQ_Initialize(pObj) != IPS_SWITCH_OK)
    {
    ...
    {
      IPS_NbInstances++;
      if (IPS_NbInstances > ISO8200BQ_INSTANCES_NBR)
      {
        status = IPS_SWITCH_ERROR;
      }
      else
...
/**
  * @brief  Deinitialize the ISO8200BQ switch
  * @param  pObj pointer to the device object
  * @retval 0 in case of success, an error code otherwise
  */
int32_t ISO8200BQ_DeInit(ISO8200BQ_Object_t *pObj)
{
  ...
  if (pObj->is_initialized == 1U)
  {
    ...
    {
      (void)ISO8200BQ_SetPwmEnable(pObj, (uint8_t)loop, 0U);
    }

     /* Disable all channels */
    (void)ISO8200BQ_SetAllChannelStatus(pObj, 0U);
...
/**
  * @brief  Get ISO8200BQ switch capabilities
  * @param  pObj pointer to the device object
...
int32_t ISO8200BQ_GetCapabilities(ISO8200BQ_Object_t *pObj, ISO8200BQ_Capabilities_t *pCapabilities)
{
  ...
  if (pObj->chipId == ISO8200BQ_CHIP_ID)
  {
    pCapabilities->nbChannels = ISO8200BQ_MAX_NB_CHANNELS;
    pCapabilities->nbCtrlPins = ISO8200BQ_MAX_NB_CONTROLS;
    pCapabilities->firstCtrlPin = ISO8200BQ_0_FIRST_CONTROL;
  }
  else
  {
    status = IPS_SWITCH_ERROR;
...
int32_t ISO8200BQ_PwmTick(ISO8200BQ_Object_t *pObj)
{
  ...
  if (pObj->isPwmEnabled != 0U)
  {
    ...
    if (pObj->ctrlMode == (uint8_t)ISO8200BQ_SCM)
    {
      flagToBeChanged = 0U;
    }
    ...
    if (pObj->ctrlMode == (uint8_t)ISO8200BQ_SCM)
    {
      if (chanEnBitmapChangeFlag != 0U)
      {
        ...
        ret = pObj->IO.WritePin(&pObj->Pin, ISO8200BQ_0_CTRL_SYNCH, 1U);
        if (ret != IPS_SWITCH_OK)
        {
          status = IPS_SWITCH_ERROR;
        }
        ret = pObj->IO.WritePin(&pObj->Pin, ISO8200BQ_0_CTRL_LOAD, 1U);
        if (ret != IPS_SWITCH_OK)
        {
          status = IPS_SWITCH_ERROR;
        }
        for (loop = 0U; loop < ISO8200BQ_TW_LOAD_WIDTH; loop++)
        {
          /* Loop needed to match tW(LOAD) */
          ret = pObj->IO.WritePin(&pObj->Pin, ISO8200BQ_0_CTRL_LOAD, 0U);
          if (ret != IPS_SWITCH_OK)
          {
            status = IPS_SWITCH_ERROR;
          }
    ...
    if (pObj->ctrlMode == (uint8_t)ISO8200BQ_SCM)
    {
      if (chanEnBitmapChangeFlag != 0U)
      {
        ...
        for (loop = 0U; loop < ISO8200BQ_TSU_LOAD_WIDTH; loop++)
        {
        ...
        for (loop = 0U; loop < ISO8200BQ_TW_SYNC_WIDTH; loop++)
        {
          /* Loop needed to keep SYNC low for more than 20 us, that is tw(SYNC) */
          ret = pObj->IO.WritePin(&pObj->Pin, ISO8200BQ_0_CTRL_SYNCH, 0U);
          if (ret != IPS_SWITCH_OK)
          {
            status = IPS_SWITCH_ERROR;
          }
...
int32_t ISO8200BQ_SetAllChannelStatus(ISO8200BQ_Object_t *pObj, uint8_t ChanStatusBitmap)
{
  ...
  else
  {
    ...
    if (pObj->ctrlMode == (uint8_t)ISO8200BQ_SCM)
    {
      ...
      for (waitLoop = 0U; waitLoop < ISO8200BQ_TW_LOAD_WIDTH; waitLoop++)
      {
        /* Loop needed to match tW(LOAD) */
        ret = pObj->IO.WritePin(&pObj->Pin, ISO8200BQ_0_CTRL_LOAD, 0U);
        if (ret != IPS_SWITCH_OK)
    ...
    if (pObj->ctrlMode == (uint8_t)ISO8200BQ_SCM)
    {
      ...
      /* SYNCH low (Preparing to transfer to output stage) */
      /* SYNCH high (Effective transfer to output stage) */
      for (waitLoop = 0U; waitLoop < ISO8200BQ_TSU_LOAD_WIDTH; waitLoop++)
      {
        /* Loop needed to match tSU(LOAD) */
        ret = pObj->IO.WritePin(&pObj->Pin, ISO8200BQ_0_CTRL_LOAD, 1U);
        if (ret != IPS_SWITCH_OK)
      ...
      for (waitLoop = 0U; waitLoop < ISO8200BQ_TW_SYNC_WIDTH; waitLoop++)
      {
        /* Loop needed to keep SYNC low for more than 20 us, that is tw(SYNC) */
        ret = pObj->IO.WritePin(&pObj->Pin, ISO8200BQ_0_CTRL_SYNCH, 0U);
        if (ret != IPS_SWITCH_OK)
...
int32_t ISO8200BQ_SetChannelFreq(ISO8200BQ_Object_t *pObj, uint8_t ChanId, uint16_t ChanFreq)
{
  ...
  {
    uint16_t newFreq;
    if (ChanFreq > ISO8200BQ_MAX_CHAN_FREQ)
    {
       newFreq = ISO8200BQ_MAX_CHAN_FREQ;
    }
    else
    {
      newFreq = ChanFreq;
...
int32_t ISO8200BQ_SetOperatingMode(ISO8200BQ_Object_t *pObj, uint8_t *pOpMode)
{
  int32_t status = IPS_SWITCH_OK;
  int32_t ret;
  uint8_t CtrlPinStatus;

  if (ISO8200BQ_GetCtrlPinStatus(pObj, ISO8200BQ_0_CTRL_ISO_DRV, &CtrlPinStatus) != IPS_SWITCH_ERROR)
  {
    if (CtrlPinStatus != 0U)
    {
      /* ISO_DRV level is high: SCM (SW36 closed 2-3, SW37 closed 2-3) */
      pObj->ControlPinsBitmap |= (0x1U << (ISO8200BQ_0_CTRL_ISO_DRV - pObj->firstCtrl));
      pObj->ctrlMode = (uint8_t)ISO8200BQ_SCM;
      *pOpMode = (uint8_t)pObj->ctrlMode;
      ret = pObj->IO.WritePin(&pObj->Pin, ISO8200BQ_0_CTRL_SYNCH, 1U);
      if (ret != IPS_SWITCH_OK)
      {
        status = IPS_SWITCH_ERROR;
    ...
    {
      /* ISO_DRV level is low: DCM (SW36 closed 1-2, SW37 closed 2-3) */
      pObj->ControlPinsBitmap &= ~(0x1U << (ISO8200BQ_0_CTRL_ISO_DRV - pObj->firstCtrl));
      pObj->ctrlMode = (uint8_t)ISO8200BQ_DCM;
      *pOpMode = (uint8_t)pObj->ctrlMode;
      ret = pObj->IO.WritePin(&pObj->Pin, ISO8200BQ_0_CTRL_SYNCH, 0U);
      if (ret != IPS_SWITCH_OK)
      {
  ...
  {
    *pOpMode = (uint8_t)ISO8200BQ_NONE;
    status = IPS_SWITCH_ERROR;
  }
...
static int32_t ISO8200BQ_Initialize(ISO8200BQ_Object_t *pObj)
{
  ...
  if (ISO8200BQ_SetAllChannelStatus(pObj, 0U) != IPS_SWITCH_OK)
  {
    ret = IPS_SWITCH_ERROR;
  }
  return ret;
