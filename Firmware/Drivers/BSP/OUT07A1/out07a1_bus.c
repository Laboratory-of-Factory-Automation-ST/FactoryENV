...
/** @defgroup OUT07A1_BUS_Global_Variables OUT07A1_BUS Global Variables
 * @{
 */
...
/** @defgroup OUT07A1_BUS_Functions_Definition OUT07A1_BUS Functions Definition
 * @{
 */
...
/**
  ...
  * @param  Pin Pointer to pin structure
  * @param  FaultPinStatus Pointer to the Fault pin status (LSB) and Input pin status (MSB)
  * @retval 0 in case of success, an error code otherwise
  */
int32_t OUT07_ReadFaultPin(IPS_SWITCH_Pins_t *Pin, uint16_t* FaultPinStatus)
{
  ...
  *FaultPinStatus = ((uint8_t)(HAL_GPIO_ReadPin(Pin->FLT_L_GPort, Pin->FLT_L_GPin))
                   |((uint8_t)HAL_GPIO_ReadPin(Pin->OL_L_GPort, Pin->OL_L_GPin) << 1U)
                   |((uint8_t)HAL_GPIO_ReadPin(Pin->DIAG1_L_GPort, Pin->DIAG1_L_GPin) << 2U)
                   |((uint8_t)HAL_GPIO_ReadPin(Pin->DIAG2_L_GPort, Pin->DIAG2_L_GPin) << 3U)
                   |((uint8_t)HAL_GPIO_ReadPin(Pin->DIAG3_L_GPort, Pin->DIAG3_L_GPin) << 4U)
                   |((uint8_t)HAL_GPIO_ReadPin(Pin->DIAG4_L_GPort, Pin->DIAG4_L_GPin) << 5U));
  InPinStatus = (uint8_t)(HAL_GPIO_ReadPin(Pin->IN1_GPort, Pin->IN1_GPin))
              | ((uint8_t)HAL_GPIO_ReadPin(Pin->IN2_GPort, Pin->IN2_GPin) << 1U)
              | ((uint8_t)HAL_GPIO_ReadPin(Pin->IN3_GPort, Pin->IN3_GPin) << 2U)
              | ((uint8_t)HAL_GPIO_ReadPin(Pin->IN4_GPort, Pin->IN4_GPin) << 3U);
  *FaultPinStatus &= 0xFF;
  *FaultPinStatus |= (uint16_t)(InPinStatus << 8U);
